        ...
        try:
            ...
            if "name" and "url_service" and "driver" in req_body:
                ...
                if len(url_service) == 0 or len(name_service) == 0:
                    return make_response(
                        jsonify(
                            {
                                "success": False,
                                "body": "La propiedad url_service y/o name está vacía, por favor verificar.",
                            }
                        ),
                        400,
                    )
                ...
                if req_body["driver"] == "shp":
                    ...
                    response = to_geojson(url_service, name_service, project)
                    try:
                        deleteAllFiles("temp")
                    except Exception as e:
                        print(e)
                ...
                return make_response(
                    jsonify(
                        {
                            "success": False,
                            "body": "No se recibieron las propiedades name, driver y/o url_service",
                        }
                    ),
                    400,
                )
